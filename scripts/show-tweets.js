...
const tweet2 = () => {
    ...
    document.querySelector('#tweet3 .message').innerHTML = tweets.statuses[1].text
    document.querySelector('#tweet3 .username').innerHTML = tweets.statuses[1].user.screen_name
    document.querySelector('#tweet3 .date').innerHTML = tweets.statuses[1].created_at
}
...
const showAll = () => {
    ...
    //.....
    newDiv = document.createElement('div')
    newContent = document.createTextNode(tweets.statuses[2].text)
    newDiv.appendChild(newContent) //add the text node to the newly created div.
    document.getElementById('all').appendChild(newDiv)
}
